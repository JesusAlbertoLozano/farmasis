...
function busca()
{
  ...
  if (f.country.value == "")
  { alert("Ingrese el nombre del Producto"); f.country.focus(); return; }
  f.target = "_top";
  f.method = "post";
...
<table class="tabla2" width="358" border="0" bgcolor="#FFFFCC">
  <tr>
	...
	<form id="form1" name="form1" onkeyup="highlight(event)" onclick="highlight(event)" method = "post" target="_top" action="../mov_prod.php">
      <div>
        <input height="100px" name="country" type="text" id="country" onkeyup="ajax_showOptions(this,'getCountriesByLetters',event)" size="52" onclick="this.value=''"/>
...
<?php if ($val == 1)
{
$sql="SELECT codpro,desprod FROM producto where desprod like '%$buscar%'";
}
else
{
$sql="SELECT codpro,desprod FROM producto where desprod like '%$prod%'";
}
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result) ){
while ($row = mysqli_fetch_array($result)){
...
?>
  <tr onmouseover="this.style.backgroundColor='#FFFF99';this.style.cursor='hand';" onmouseout="this.style.backgroundColor='#ffffff';">
    ...
    <td width="41"><a href="../mov_prod.php?codpro=<?php echo $codpro?>&search=1&word=<?php echo $buscar?>" target="_top"><img src="../../../images/lens.gif" width="15" height="16" border="0"/></a></td>
  </tr>
<?php }
}
}
