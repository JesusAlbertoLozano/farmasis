...
function buscar()
{
	  ...
	  if (tip ==1)
	  {
	  f.action = "marcaaas1.php";
	  }
	  else
	  {
	  f.action = "marcaaas_prog.php";
	  }
	  f.submit();
}
function salir()
...
if ($val == 1)
{
	...
	if ($tipo1 == 2)
	{
            $sql="SELECT desprod FROM producto inner join titultabladet on producto.codmar = titultabladet.codtab where $tabla > 0 and destab between '$marca1' and '$marca2' between $cat1 and $cat2 order by codpro";
	}
	if ($tipo1 == 3)
	{
            $sql="SELECT desprod FROM producto inner join titultabladet on producto.codmar = titultabladet.codtab where $tabla1 > 0 and destab between '$marca1' and '$marca2' between $cat1 and $cat2 order by desprod";
	}
	if ($tipo1 == 4)
	{
            $sql="SELECT desprod FROM producto inner join titultabladet on producto.codmar = titultabladet.codtab where $tabla1 = 0 and destab between '$marca1' and '$marca2' between $cat1 and $cat2 order by desprod";
	...
	if ($tipo1 == 5)
	{
            $sql="SELECT desprod FROM producto inner join titultabladet on producto.codmar = titultabladet.codtab where $tabla > 0 and $tabla1 > 0 and destab between '$marca1' and '$marca2' between $cat1 and $cat2 order by codpro";
	}
	if ($tipo1 == 6)
	{
            $sql="SELECT desprod FROM producto inner join titultabladet on producto.codmar = titultabladet.codtab where $tabla > 0 and $tabla1 = 0 and destab between '$marca1' and '$marca2' between $cat1 and $cat2 order by codpro";
...
<!--             <div id="categoria" name="categoria"  >
             ...
             <td width="5"> DESDE</td>
			  ...
			  <select name="cat1" id="cat1" onchange="habilitar(this.value);" >
                            ...
                            <?php
                                            ...
                                            $destab1	= $row['destab'];
                                            ?>                            
                            <option  value="1" value="<?php echo $row['destab'];?>" <?php if ($destab1 == $cat1){?> selected="selected"<?php }?>><?php echo $row['destab'];?></option>
                            <?php 
                                            } 
				?>
                          </select>
...
<!--                  <option value="3" <?php if ($tipo1 == 3){?>selected="selected"<?php }?>>TODOS LOS PRODUCTOS PERO QUE TENGAN STOCK MINIMO </option>
			  ...
			  {
			  ?>
			  <a href="marcas1.php?val=<?php echo $val?>&tipo=<?php echo $tipo?>&tipo1=<?php echo $tipo1?>&ltdgen=<?php echo $ltdgen?>&local=<?php echo $local?>&inicio=<?php echo $inicio?>&registros=<?php echo $registros?>&pagina=<?php echo $pagina-1?>&marca1=<?php echo $marca1?>&marca2=<?php echo $marca2?>">
			  <img src="../../images/play1.gif" width="16" height="16" border="0"/>			  </a>
			  <?php }*/
			  ?>			  </td>
			  <td width="17">
			  ...
			  ?>
			  <a href="marcas1.php?val=<?php echo $val?>&tipo=<?php echo $tipo?>&tipo1=<?php echo $tipo1?>&ltdgen=<?php echo $ltdgen?>&local=<?php echo $local?>&inicio=<?php echo $inicio?>&registros=<?php echo $registros?>&pagina=<?php echo $pagina+1?>&marca1=<?php echo $marca1?>&marca2=<?php echo $marca2?>">
			  <img src="../../images/play.gif" width="16" height="16" border="0"/>			  </a>
			  <?php }*/
			  ?>			  </td>
            </tr>
          </table>
