...
if(isset($_REQUEST['getCountriesByLetters']) && isset($_REQUEST['letters'])){
	...
	*/
	...
	if($t == 0)
	{
		...
		{
		$res = mysqli_query($conexion,"select P.codpro,P.desprod,TD.destab from producto P inner join titultabladet TD on TD.codtab = P.codmar where P.desprod like '".$letters."%' or P.codpro like '".$letters."'") or die(mysqli_error());
		}
	}
	else
	{
	...
	while($inf = mysqli_fetch_array($res)){
		echo $inf["codpro"]."###"."<b>COD = </b>" . $inf["codpro"] . " - <b>PROD = </b>" .$inf["desprod"].'-Marca: '.$inf["destab"]."|";
	}	
}
?>
