...
if ($SoloCompras == 1)
{
    $sql="SELECT fecha,codkard,nrodoc,tipmov,tipdoc,qtypro,fraccion,factor,sactual,invnum,preciocompra,eliminado FROM kardex where fecha between '$date1' and '$date2' and codpro = '$country_ID' and sucursal = '$local' and tipmov = 1 and estado2 <> '1' and tipdoc = 1 order by fecha,codkard";
}
else
{
    $sql="SELECT fecha,codkard,nrodoc,tipmov,tipdoc,qtypro,fraccion,factor,sactual,invnum,preciocompra,eliminado FROM kardex where fecha between '$date1' and '$date2' and codpro = '$country_ID' and sucursal = '$local' and estado2 <> '1' order by fecha,codkard";
}
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result)){
while ($row = mysqli_fetch_array($result)){
...
?>
   ...
   <td>
       <?php echo $destab_empresa; ?>
        ...
        <td width="55"><div align="left"><?php echo $signo;?></div></td>
