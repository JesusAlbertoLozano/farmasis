...
<table width="930" border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	if ($tipo1 == 1)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where destab between '$marca1' and '$marca2' order by codpro";
	}
	if ($tipo1 == 2)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where $tabla > 0 and destab between '$marca1' and '$marca2' order by codpro";
	}
	if ($tipo1 == 3)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where $tabla1 > 0 and destab between '$marca1' and '$marca2' order by desprod";
	}
	if ($tipo1 == 4)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where $tabla1 = 0 and destab between '$marca1' and '$marca2' order by desprod";
	...
	if ($tipo1 == 5)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where $tabla > 0 and $tabla1 > 0 and destab between '$marca1' and '$marca2' order by codpro";
	}
	if ($tipo1 == 6)
	{
	$sql="SELECT desprod,codcatp,stopro,prevta,$tabla,factor FROM producto inner join titultabladet on producto.codcatp = titultabladet.codtab where $tabla > 0 and $tabla1 = 0 and destab between '$marca1' and '$marca2' order by codpro";
