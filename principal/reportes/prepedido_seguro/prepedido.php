        ...
        foreach ($text as $key => $n) {
            ...
            //if(($ncontrolx <> 0)&&($ncontrolx <>'')){
             ...
             if (($ncontrolx <= $control)) {
                ...
                $sql2 = "UPDATE detalle_prepedido DP SET solicitado='0' WHERE DP.idprepedido = '$idpreped' AND DP.iddetalle='$key'";
                                            ...
                                            <table width="100%" border="1" align="center" id="tabDetalle" name="tabDetalle" cellpadding="0" cellspacing="0" style="
                                                ...
                                                <tbody>
                                                    ...
                                                    //$sql="SELECT * FROM detalle_prepedido DP,producto P where DP.idprod=P.codpro AND DP.numpagina='$numpagina'order by iddetalle asc";
                                                     $sql = "SELECT DP.iddetalle,DP.idprepedido,DP.idcantidad,DP.solicitado,DP.fraccion,P.desprod,M.destab,P.blister,P.codmar,P.s000,P.factor,P.stopro,P.$columna as columna,P.prevta,P.codpro,P.utlcos FROM detalle_prepedido as DP INNER JOIN producto as P on DP.idprod=P.codpro inner join titultabladet AS M ON M.codtab=P.codmar WHERE DP.numpagina='$numpagina' AND M.tiptab = 'M'   AND  DP.solicitado <>'' AND  DP.solicitado <>'0' and P.s000 >0 order by M.destab, P.desprod ";
//                                                    error_log($sql);
                                                    $result = mysqli_query($conexion, $sql);
                                                    $cont = 0;
                                                    if (mysqli_num_rows($result)) {
                                                        while ($row = mysqli_fetch_array($result)) {
                                                            ...
                                                            $desprod = $row['desprod'];
                                                            $codmar = $row['codmar'];
                                                            $stocentpro = (isset($row['s000']) ? $row['s000'] : 0);
                                                            $factor = $row['factor'];
                                                            $stopro = $row['stopro'];
