...
if (isset($_REQUEST['getCountriesByLetters']) && isset($_REQUEST['letters'])) {
    ...
    if ($t == 0) {
        ...
        if (strpos($letters, $caracter) !== false) {
            $res = mysqli_query($conexion, "select codpro,desprod,codmar,$columna,factor from producto where codpro  like '" . $letters . "' and eliminado='0'  order by desprod limit $limit") or die(mysqli_error());
        } else {
