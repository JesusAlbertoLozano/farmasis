...
$sql="SELECT numdoc,invtot,sucursal FROM movmae where invnum = '$cod'";	
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result)){
	while ($row = mysqli_fetch_array($result)){
		$numdoc    = $row['numdoc'];
...
if (mysqli_num_rows($result)){
	...
	//				$sactual   = $stopro;
		...
		$sql1 = "INSERT INTO kardex (nrodoc,codpro,fecha,tipmov,tipdoc,qtypro,fraccion,factor,invnum,usecod,sactual,sucursal) values ('$numdoc','$codpro','$fechaActual','$tipmov','$tipdoc','$qtypro','$qtyprf','$factor','$transferencia_ing','$usuario','$sactual','$codloc')";
		mysqli_query($conexion,$sql1);
		if (mysqli_errno($conexion))
		...
		if (mysqli_num_rows($result1)){
			while ($row1 = mysqli_fetch_array($result1)){
				...
				$sql1 = "INSERT INTO movmov (invnum,invfec,codpro,qtypro,qtyprf,pripro,costre,numlote,costpr) values ('$transferencia_ing','$fechaActual','$codpro','$qtypro2','$qtyprf2','$pripro2','$costre2','$numlote','$costpr')";
				mysqli_query($conexion,$sql1);
				if (mysqli_errno($conexion))
...
$sql1 = "UPDATE movmae set estado = '1', proceso = '0',invtot='$invtot',monto='$invtot',sucursal1='$sucursal',hora='$hoy'   where invnum = '$transferencia_ing'";
mysqli_query($conexion,$sql1);
if (mysqli_errno($conexion))
	error_log("Actualiza Estado Mae SQL(".$sql1.")\nError(".mysqli_error($conexion).")");
