...
//				$sactual   = $stopro;
		...
		mysqli_query($conexion,"INSERT INTO kardex (nrodoc,codpro,fecha,tipmov,tipdoc,qtypro,fraccion,factor,invnum,usecod,sactual,sucursal) values ('$numdoc','$codpro','$fechaActual','$tipmov','$tipdoc','$qtypro','$qtyprf','$factor','$cod','$usuario','$sactual','$codloc')")	;

		/////////////////////////////////////////////////
mysqli_query($conexion,"INSERT INTO movmov (invnum,invfec,codpro,qtypro,qtyprf,pripro,costre,costpr,numlote) values ('$cod','$fechaActual','$codpro','$qtypro','$qtyprf','$pripro','$costre','$costpr','$numlote')")	;
}
}
}
mysqli_query($conexion,"DELETE from tempmovmov where invnum = '$cod'")	;
mysqli_query($conexion,"UPDATE movmae set invtot  = '$mont2', monto = '$mont2',refere = '$referencia', codusu = '$vendedor', estado = '0', proceso = '0',hora='$hoy'  where invnum = '$cod'");
echo "<script>if(confirm('Desea imprimir esta transferencia?')){document.location='transferencia1_sal_op_reg1.php';}else{ alert(document.location='../ing_salid.php');}</script>";
?>