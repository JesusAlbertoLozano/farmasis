...
<head>
    ...
    $sql = "SELECT anular_venta_vendedor,anularImpresion FROM datagen";
    $result = mysqli_query($conexion, $sql);
    if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_array($result)) {
            $anular_venta_vendedor = $row['anular_venta_vendedor'];
            $anularImpresion = $row['anularImpresion'];
        }
    }
    ...
    if (mysqli_num_rows($result1X)) {
     ...
     if ($anularImpresion == 1) {
        $sql1Xim = "SELECT codgrup FROM grupo_user where codgrup = '$codgrup' AND (NOMGRUP LIKE '%ADMINISTRADOR%')";
    } else {
        $sql1Xim = "SELECT codgrup FROM grupo_user where codgrup = '$codgrup' AND (NOMGRUP LIKE '%ADMINISTRADOR%')";
    }
    $result1Xim = mysqli_query($conexion, $sql1Xim);
    if (mysqli_num_rows($result1Xim)) {
        $PermisoDELI = 1; //PERMISO PARA BORRAR
    } else {
        $PermisoDELI = 0; //NO TIENE PERMISO PARA BORRAR
    }
        ...
        <table width="100%" border="0">
            <tr>
                <td width="938">
                    ...
                    if ($tip == 1) {
                        if (!is_numeric($num[0])) {
                            $sql = "SELECT nrofactura,invnum,invfec,cuscod,codmed,usecod,nrovent,bruto,valven,invtot,igv,forpag,val_habil,estado,hora,sucursal,correlativo,tipdoc FROM venta where (nrofactura = '$num' or invnum = '$num' ) and estado = '0' and tipvta = '0' and sucursal = '$codigo_local'";
                        } elseif ($nombre_local == 'LOCAL0') {
                            $sql = "SELECT nrofactura,invnum,invfec,cuscod,codmed,usecod,nrovent,bruto,valven,invtot,igv,forpag,val_habil,estado,hora,sucursal,correlativo,tipdoc FROM venta where (invnum = '$num' or nrofactura = '$num') and estado = '0' and tipvta = '0' and sucursal = '$codigo_local'";
                        } else {
                            $sql = "SELECT nrofactura,invnum,invfec,cuscod,codmed,usecod,nrovent,bruto,valven,invtot,igv,forpag,val_habil,estado,hora,sucursal,correlativo,tipdoc FROM venta where (invnum = '$num' or nrofactura = '$num') and estado = '0' and sucursal = '$codigo_local' and tipvta = '0'";
                        }
                    } else {
                    ...
                    ?>
                        <table width="100%" border="0">
                            ...
                            <tr>
                                ...
                                <td colspan="2"><input name="textfield22" type="text" size="15" disabled="disabled" value="<?php if ($codcolegiatura2 <> "") {
                            ...
                            <tr>
                                ...
                                <td width="580">
                                    <div align="right">
                                        ...
                                        <input tabindex="21" name="printeremail" type="button" id="printeremail" value="Reimprimir email" title="Se le enviara el comprobante al correo electronico proporcionado por el cliente" class="grabarventa" onClick="email('<?php echo $correo; ?>');" />
                                    </div>
                                </td>
                            </tr>
                        </table>
