...
$invnum     = (isset($_SESSION['compras']) ? $_SESSION['compras'] : "");;
$ok         = $_REQUEST['ok'];
$ckigv      = $_REQUEST['ckigv'];
$busca_prov = (isset($_REQUEST['busca_prov']) ? $_REQUEST['busca_prov'] : "");;
$busca_empre = $_REQUEST['busca_empre'];
?>
...
function validar_bonif()
{
	...
	if (document.form2.mesL.value == "") {
        alert("Ingrese el Mes");
        return;
    }
    if (document.form2.yearsL.value == "") {
    ...
    var mesm = (dt.getMonth() + 1);
    var yearsm = dt.getFullYear();
    
    var mesv =  document.form2.mesL.value;
    var yearsv = document.form2.yearsL.value;

    if(yearsm > yearsv)
    {
        alert("Ingrese un a�o mayor");
...
<form id="form2" name="form2" onKeyUp="highlight(event)" onClick="highlight(event)">
    ...
    <input type="hidden" id="busca_empre" name="busca_empre" value="<?php echo $busca_empre;?>"/>
	...
	$sqlq="SELECT codpro,canbon,tipbon, numlote,vencim FROM tempmovmov_bonif where codtemp = '$codtemp' and invnum = '$invnum'";
	$resultq = mysqli_query($conexion,$sqlq);
	if (mysqli_num_rows($resultq)){
	while ($rowq = mysqli_fetch_array($resultq)){
			...
			$listadoVencim = explode("/", $vencim);
			$mes = $listadoVencim[0];
			$year = $listadoVencim[1];
	}	
	}
	else
	{
	...
	?>
<table width="100%" border="0" class="celda2">
  <tr>
...
<table width="100%" border="0" class="celda2">
  <tr>
      <td width="86"><strong>NUM LOTE</strong></td>
      <td width="510">
        <input name="numloteL" type="text" id="numloteL" size="20" value="<?=$numlote?>" />
      </td>
      <td width="63"><div align="right"><strong>VENCIMIENTO</strong></div></td>
      
   
      <td width="260">
        
        <input name="mesL" type="text" id="mesL" size="4" maxlength="2" placeholder="MM" value="<?php echo $mes ?>" onkeypress="return acceptNum(event)"  />
        /
        <input name="yearsL" type="text" id="yearsL" size="2" maxlength="4" placeholder="AAAA" value="<?php echo $year ?>" onkeypress="return acceptNum(event)" />
...
<form id="form1" name="form1" onKeyUp="highlight(event)" onClick="highlight(event)">
    ...
    <table class="celda2" width="100%">
        <tr>
            <th width="284" bgcolor="#50ADEA" class="titulos_movimientos">DESCRIPCION</th>
                <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">CANT</div></th>
                <th width="33" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">BONIF</div></th>
                <th width="150" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">MARCA</div></th>
                <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">P. UNIT</div></th>
                <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">DESC1</div></th>
                <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">DESC2</div></th>
                <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">DESC3</div></th>
            <th width="43" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">P. VTA</div></th>
                <th width="61" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">SUB TOT</div></th>
            <th width="32" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">PREC</div></th>
                <th width="27" bgcolor="#50ADEA" class="titulos_movimientos"><div align="center">LOTE</div></th>
                <th width="38" bgcolor="#50ADEA" class="titulos_movimientos">&nbsp;</th>
        </tr>
    <?php $i = 0;
	while ($row = mysqli_fetch_array($result)){
			$i++;
			...
			$sql1="SELECT desprod,codmar,factor,igv,pcostouni,stopro,lotec FROM producto where codpro = '$codpro'";
			$result1 = mysqli_query($conexion,$sql1);
			if (mysqli_num_rows($result1)){
			while ($row1 = mysqli_fetch_array($result1)){
	...
	?>
	 <tr  bgcolor="<?php echo $bg;?>" onMouseOver="this.style.backgroundColor='<?php echo $over;?>';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='<?php echo $out;?>';">
            <td valign="bottom">
		<a href="javascript:popUpWindow('ver_prod.php?cod=<?php echo $codpro?>&invnum=<?php echo $invnum?>', 435, 110, 560, 200)" title="EL FACTOR ES <?php echo $factor?>"><?php echo substr($desprod,0,65); echo "";?></a>
            </td>
		...
		<input type="hidden" name="factor" value="<?php echo $factor;?>"/>
                ...
                <?php } else { if ($qtyprf <> ""){ echo $qtyprf; } else {echo $qtypro;}}?></div>
            </td>
            <td >
                <center>
                    <?php if ($canbon <> 0){?>
                    <a href="compras3.php?codtemp=<?php echo $codtemp?>&codprod=<?php echo $codpro?>&bon=1&ok=<?php echo $ok?>&busca_prov=<?php echo $busca_prov;?>&busca_empre=<?php echo $busca_empre;?>">
            ...
            <td   valign="bottom">
                <div align="center">
                    ...
                    <?php } else { ?>
                    <a href="compras3.php?cod=<?php echo $codpro?>&valform=1&ok=<?php echo $ok?>&busca_prov=<?php echo $busca_prov;?>&busca_empre=<?php echo $busca_empre;?>">
                        <img src="../../../images/edit_16.png" width="16" height="16" border="0"/>
                        </a>
                        <a href="compras4.php?cod=<?php echo $codtemp?>&ok=<?php echo $ok?>&busca_prov=<?php echo $busca_prov;?>&busca_empre=<?php echo $busca_empre;?>" target="comp_principal">
