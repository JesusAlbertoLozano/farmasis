...
if ($search == 1)
{
	$sql="SELECT codpro,desprod,codbar,factor,margene,stopro,moneda,prevta,imapro,detpro,cant1,cant2,cant3,desc1,desc2,desc3,activo,prelis,prevta,preuni,margene,costre,stopro,codmar,coduso,codfam,igv,incentivado,blister,lotec,codpres,codcatp,activo1,costpr,utlcos FROM producto where codpro = '$busqueda' and codpro <> 0";
}
else
{
	$sql="SELECT codpro,desprod,codbar,factor,margene,stopro,moneda,prevta,imapro,detpro,cant1,cant2,cant3,desc1,desc2,desc3,activo,prelis,prevta,preuni,margene,costre,stopro,codmar,coduso,codfam,igv,incentivado,blister,lotec,codpres,codcatp,activo1,costpr,utlcos FROM producto where codpro <> 0 $limit";
}
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result) ){
	while ($row = mysqli_fetch_array($result)){
...
function copyForm5() {
	...
	if (f == "")
	{
		alert("DEBE INGRESAR UNA CATEGORIA");myPopup.document.popupForm.myTextField.focus();return;
	}
	else
	{
		var i = document.forms[0].catp.length;
		var myNewOption = new
		Option(myPopup.document.forms[0].myTextField.value, myPopup.document.forms[0].myTextField.value);
		document.forms[0].catp.options[i] = myNewOption;
		document.forms[0].catp.selectedIndex = i;
		myPopup.window.close();
		return false;
	}
}
