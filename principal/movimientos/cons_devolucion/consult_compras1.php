...
$sql="SELECT count(invnum) FROM movmae where tipmov = '1' and tipdoc = '3' and proceso = '0'";
...
if (($numero == "") && ($pag == ""))
{
$sql="SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '3' and proceso = '0'   order by numdoc desc limit 1";
//echo "1";
}
else
{	
	if (($numero == "") && ($pag <> ""))
	{
	$sql="SELECT numdoc FROM movmae where tipmov = '1' and tipdoc = '3' and proceso = '0'   order by numdoc desc $limit";
	//echo "2";
	}
	else
	{
	$sql="SELECT numdoc FROM movmae where numdoc = '$numero' and tipmov = '3' and tipdoc = '2' and proceso = '0'   order by numdoc desc";
	//echo "3";
	}
}
$result = mysqli_query($conexion,$sql);
...
<?php if (($numero == "") && ($pag == ""))
{
$sql="SELECT * FROM movmae where tipmov = '1' and tipdoc = '3' and proceso = '0'   order by numdoc desc limit 1";
}
else
{	
	if (($numero == "") && ($pag <> ""))
	{
	$sql="SELECT * FROM movmae where tipmov = '1' and tipdoc = '3' and proceso = '0'   order by numdoc $limit";
	}
	else
	{
	$sql="SELECT * FROM movmae where numdoc = '$numero' and tipmov = '1' and tipdoc = '3' and proceso = '0'   order by numdoc desc";
	}
}
$result = mysqli_query($conexion,$sql);
if (mysqli_num_rows($result)){
		  ...
		  ?>
            <table width="932" border="0">
              <tr>
                ...
                <td width="580"><label>
                    <div align="right">
                      <input name="printer" type="button" id="printer" value="Imprimir" class="imprimir" <?php if ($impresion == 0){?>disabled="disabled"<?php }?> onclick="prints(<?php echo $invnum;?>)"/>
