...
function controlador($accion, $conexion) {
    ...
    switch ($accion) {
        ...
        case 'GUARDAR_VENTA_MASIVO':
            ...
            $sqldetalle_ventaMasiva = "SELECT invnum FROM venta  as v  WHERE v.invfec = '$date' AND tipdoc='1' AND LENGTH(v.nrofactura) > 0 AND v.sucursal = '$sucursal'  and v.sunat_enviado=0  and v.sunat_respuesta_descripcion=''  ORDER BY v.correlativo ASC";
...
function FnGenerateXmlAndSend($id = 0, $conexion, $send = true) {
    ...
    $comprobante = array(
        ...
        'moneda' => 'PEN', //PEN->SOLES; USD->DOLARES
        'total_opgravadas' => number_format($total_gravada, 2, '.', ''),
...
function FnQuerySummary($date, $sucursal, $tiposummary = 'boleta') {
    ...
    $query_doc .= "               AND LENGTH(v.nrofactura)>0 AND v.sunat_enviado = '$sunat_enviado' 
            ...
            if ($insert) {
                ...
                foreach ($items as $key) {
                    ...
                    $query_boleta = "UPDATE venta SET "
                            ...
                            . "sunat_hash = '$hash',"
                            . FnQueryUpdateTicket($typesummary, $ticket, $resumen_resumen)
                            . "sisFacturacion='$enviado' "
                            . "WHERE invnum = '$boleta'";
                    mysqli_query($conexion, $query_boleta);
                    var_dump($query_boleta);
                }
            }
        }
    }
